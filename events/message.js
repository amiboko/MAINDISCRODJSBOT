...
module.exports = async (client, message) => {
  ...
  if (message.channel.type !== "dm") {
    ...
    if (client.getSettings(message.guild.id).pointsEnabled === 'true') {
      ...
      if (cooled.get(message.author.id)) return message.delete(1000).then(message.channel.send(message.author + ', אני לא עובד אצלך, תרגיע עם הבקשות').then(message => message.delete(30000)));
